class UsersController < ApplicationController
  ...
  def render_user_json_failed
    render json: {status: "failed", data: {message: @user.errors.messages.as_json}}
  end

  def render_user_json_success
    render json: {status: "success", data: {id: @user.id, name: @user.name, token: @user.token, provider: @user.provider}}
  end
